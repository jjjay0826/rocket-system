...
/* 本次開機使用的 LOG 檔名（序號制，與 CSV 同號，如 LOG_003.TXT）。
 * 預設 "log.txt" 為保底；main 開機時會 logger_set_name 改成序號檔名。
 * 此為 static 全域 → 恢復路徑（log_retry 的 logger_init）自動沿用同名
 * APPEND 接續，不會清空本次開機的紀錄。 */
static char log_name[20] = "log.txt";
...
/* 以 APPEND 開 log_name（新檔自動建），寫 BOOT 分隔線，設 sd_ok。 */
void logger_open(void) {
    fres = f_open(&file, log_name, FA_WRITE | FA_OPEN_APPEND | FA_OPEN_ALWAYS);
    if (fres != FR_OK) {
        /* 檔案損壞（FAT chain 異常）→ 強制重建，避免 FatFS 無限 loop */
        f_unlink(log_name);
        fres = f_open(&file, log_name, FA_WRITE | FA_CREATE_ALWAYS);
    }
    if (fres == FR_OK) {
        /* 開機分隔線：APPEND 模式會在同一開機檔內累積（恢復重開亦接續）*/
        UINT bw_b = 0;
        const char *bn = "\r\n===== BOOT =====\r\n";
        f_write(&file, bn, strlen(bn), &bw_b);
        f_sync(&file);
    }
...
int logger_open_for_read(void) {
    ...
    fres = f_open(&file, log_name, FA_READ);
    if (fres != FR_OK) {
        uart1_write("ERR: Cannot open log file for reading\r\n");
        return -1;
    }
    return 0;
}

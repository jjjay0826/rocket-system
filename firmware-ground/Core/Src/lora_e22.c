...
int LoRa_Receive(uint8_t *buf, uint8_t max_len)
{
    ...
    while (curr != rx_head) {
        ...
        curr = (uint16_t)((curr + 1) & LORA_RXBUF_MASK);
        data_len++;
        
        /* 保護機制：如果這包資料太長，強制當作收到一行，避免塞爆 */
    ...
    /* 2. ★ 關鍵修正 ★：如果還沒收到 '\n'，直接返回 0，絕對「不要」移動 rx_tail 吃掉字元！ */
    if (!has_newline) {
        return 0;
    }

    /* 3. 確定有完整一行了，才真正把資料讀出來並移動 rx_tail */
    uint16_t idx = 0;

...
void HAL_I2C_MspInit(I2C_HandleTypeDef* i2cHandle)
{
  ...
  /* I2C1 bus recovery: 用 GPIO 模擬 9 個 SCL 脈衝，解除 SDA stuck low
   * PB6=SCL, PB9=SDA（Open-Drain + 外部或內部上拉）               */
  {
    __HAL_RCC_GPIOB_CLK_ENABLE();
    GPIO_InitTypeDef r = {0};
    r.Pin   = GPIO_PIN_6 | GPIO_PIN_9;
    ...
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_9, GPIO_PIN_SET);   HAL_Delay(1);
  }
  ...
  /* I2C3 bus recovery: PA8=SCL, PB8=SDA */
  {
    ...
    r.Pull  = GPIO_NOPULL;
    r.Speed = GPIO_SPEED_FREQ_LOW;
    r.Pin   = GPIO_PIN_8; HAL_GPIO_Init(GPIOA, &r);
    r.Pin   = GPIO_PIN_8; HAL_GPIO_Init(GPIOB, &r);
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8, GPIO_PIN_SET);
    HAL_Delay(1);
    for (int _i = 0; _i < 9; _i++) {
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_RESET); HAL_Delay(1);
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_SET);   HAL_Delay(1);
    }
